  ...
  return (
    <section className="librarian-picks scroll-reveal scroll-reveal--soft" aria-labelledby="librarian-picks-title">
      <div className="librarian-picks__heading">
        <span className="librarian-picks__eyebrow">Rekomendasi dari pustakawan</span>
        <h2 id="librarian-picks-title">Belum punya pilihan? <em>Mulai dari sini.</em></h2>
      </div>
      {librarianPick ? (
        <div className="librarian-desk">
          <div className="librarian-desk__book-side">
            <span className="librarian-desk__tape" aria-hidden="true" />
            <span className="librarian-desk__pick-label"><FiBookmark /> Rekomendasi minggu ini</span>
          ...
          <article className="librarian-note">
            ...
            <blockquote>"Coba baca tanpa buru-buru. Kalau beberapa halaman pertama belum cocok, tidak apa-apa. Rak ini masih punya pilihan lain."</blockquote>
          ...
          <aside className="librarian-alternatives">
            <div className="librarian-alternatives__title"><span>Kalau buku utamanya sedang dipinjam...</span><p>Bu Rani menyiapkan beberapa judul lain yang suasananya masih mirip.</p></div>
            <div className="librarian-alternatives__list">
              {alternatives.map((book, index) => (
                <Link to={`/buku?keyword=${encodeURIComponent(book.title)}`} className="alternative-book" style={{ '--alternative-color': categoryPalette[(index + 1) % categoryPalette.length] }} key={book.id}>
                  ...
                  <div><small>{book.is_available ? 'Tersedia di rak' : book.inventory_status || 'Sedang dipinjam'}</small><h3>{book.title}</h3><p>{book.author}</p></div>
      ...
      ) : <div className="landing-data-empty">Rekomendasi pustakawan akan muncul setelah koleksi tersedia.</div>}
    </section>
  );
};
